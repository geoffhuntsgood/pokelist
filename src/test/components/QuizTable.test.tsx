...
import { PokemonName, TypeName } from "../../enums";
import { allPokemonList } from "../../resources/pokemon/pokemonLists";


describe("QuizTable component rendering", () => {
  const typeCheck = `${TypeName.Grass}${TypeName.Poison}`;

  test("Initialization with less than 10 items and one shown item", () => {
    render(<QuizTable showItems={["bulbasaur"]} timedOut={false} data={allPokemonList.slice(0, 2)} />);
  ...
  test("Initialization with less than 200 items", () => {
    render(<QuizTable showItems={[]} timedOut={false} data={allPokemonList.slice(0, 151)} />);
    ...
    expect(cells[449].textContent).toEqual(TypeName.Psychic);
  });

  test("Initialization with more than 200 items and timed out", () => {
    render(<QuizTable showItems={[]} timedOut={true} data={allPokemonList.slice(0, 251)} />);
    ...
    expect(cells[746].textContent).toEqual(`${TypeName.Psychic}${TypeName.Grass}`);
  });
});
